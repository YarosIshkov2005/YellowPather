...
class FileManagerApp:
    """
    Main file manager application class for Windows.

    Provides dual-pane file manager interface with navigation, search,
    and file operations in the style of classic Windows Explorer.
    """
    ...
    def __init__(self, root) -> None:
        ...
        """
        self.root = root
        self.root.title('YellowPather File Manager (version 1.0.5)')
        self.root.geometry('760x510+320-110')
        ...
        self._states = {'create': 'Folder', 'sorting': 'Standart', 
            'protect': 'Protected', 'insert': "Don't insert"}
        ...
        self._select = {'points': []}
        self._breadcrumbs = {'points': []}
        self._positions = {'index': 0}
        
        self.insert_path = str(Path(__file__).parents[2])
    ...
    def load_resources(self) -> None:
        ...
        try:
            ...
            if (not self._factory.app_perms.check_permission(parent_catalog) 
                and not self._factory.app_state.no_message_show):
            ...
            self._factory.path_canonize.set_paths_callback('render')
        except Exception as e:
            showerror(title='Yellow Pather Error 002:', 
                message=f'Load Error: {traceback.format_exc()}', parent=self.root)
