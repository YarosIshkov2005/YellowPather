...
## YellowPather version 1.0.1 "Yellow Brick Road"
...
### New Features 🛠️:
1. **cmd** 🖥️: Fixed error in path construction (cmd now uses the Pointer class).
2. **settings** ⚙️: Improved interface, added check for forbidden characters, as well as Delete/Rename buttons for deleting and renaming directories and files.
3. **navigator** 🎚️: Improved navigation, as well as remembering the selected position.
...
- **Errors:** There is a known issue that occurs during operation execution. This happens because the command parser cannot find one of the specified points in the `path_manager.abs_paths` list if the path to the source or destination was constructed incorrectly. **Fixed in version 1.0.1.**
...
- `cmd:/` has a known issue related to constructing the absolute path to the source object, which prevents it from being found in the `abs_paths` list.
  » **Solution:** Use the system explorer or any other one. The issue will be fixed in version 1.0.1; path construction will be handled through the `Pointer` class from YPACM. **Fixed in version 1.0.1.**
- `rename_window`: YellowPather does not have built-in protection against reserved characters like `\ / : * ? " < > |` when creating new objects. **Added in version 1.0.1.**
