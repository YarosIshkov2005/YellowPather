...
class SettingsCore:
    ...
    def __init__(self, globals, callstack) -> None:
        ...
        self.open_file_callback: Any = None
    ...
    def create_window(self) -> None:
        ...
        self.app_state.settings_button_active = True
        self.mdefs._mdefs_framework.callbacks['settings'] = self.settings_window
        self.control_open_button()
        self.control_create_button()
        self.control_delete_button()
        self.control_rename_button()

    def control_create_button(self):
    ...
    def control_delete_button(self):
        if self.app_state.is_recursive_search:
            self._settings_gui.delete_button.config(state='disabled')
            return

        if self.path_manager.short_names:
            self._settings_gui.delete_button.config(state='normal')
        else:
            self._settings_gui.delete_button.config(state='disabled')

    def control_rename_button(self):
        if self.app_state.is_recursive_search:
            self._settings_gui.delete_button.config(state='disabled')
            return
            
        if self.path_manager.short_names:
            self._settings_gui.rename_button.config(state='normal')
        else:
            self._settings_gui.rename_button.config(state='disabled')
    ...
    def close_window(self) -> None:
        ...
        self.globals['update'].canonizer('render')
