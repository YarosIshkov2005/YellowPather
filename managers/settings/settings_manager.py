...
class SettingsManager:
    ...
    def create_new_resource(self, new_path: str = ''):
        try:
            catalog_path = self.path_manager.create_path
            create_path = catalog_path / os.path.basename(new_path)

            self.mdefs._mdefs_framework.bootstrap(create_path, self.create_type)
    ...
    def delete_select_resource(self):
        try:
            catalog_path = self.path_manager.delete_path
            delete_path = catalog_path
    ...
    def rename_file(self, user_input: str):
        try:
            catalog_path = self.path_manager.rename_path
            rename_path = catalog_path.parent / user_input

            self.mdefs._mdefs_framework.rename_file_callback(catalog_path, rename_path)
            self._rename_window.close_window()
        except Exception as e:
            showerror(title='Yellow Pather Error 009:', message=f'{e}', parent=self.settings_window)
